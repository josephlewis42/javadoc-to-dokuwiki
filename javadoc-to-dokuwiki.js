/**
 * Generate Markdown from your Javadoc
 *
 * Usage: nodejs javadoc-to-dokuwiki.js <your java file>
 * The resulting wikisyntax will be put in stdout
 */
var JavadocToMarkdown = function () {
    ...
    function fromDoc(code, headingsLevel, fnAddTagsMarkdown) {
        ...
        //out.push("=".repeat(headingsLevel)+" Documentation"+"=".repeat(headingsLevel));
    ...
    function fromSection(section, headingsLevel, fnAddTagsMarkdown) {
        ...
        out.push("-".repeat(80));
        out.push("\n\n");
        out.push("=".repeat(5 - headingsLevel)+" "+field+" "+"=".repeat(5 - headingsLevel));
    ...
    function replaceHTMLWithMarkdown(html) {
        var out= html.replace("<pre>","<code>").replace("</pre>", "</code>")
    ...
    function getDocDescription(docLines) {
        ...
        return cleanLine(line.replace(/<(\/)?p>/gi, "\n\n"));
    }
...
if(file === undefined) {
    console.log("Usage nodejs thisscript.js java_source_file.java")
} else {
    fs.readFile(file, {encoding: 'utf-8'}, function(err,data){
        if (!err){
            var javadocToMarkdown = new JavadocToMarkdown();
            var output = javadocToMarkdown.fromJavadoc(data, 1);
            console.log(output);
